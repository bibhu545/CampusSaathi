...
<?php
	...
	$id = $_REQUEST['did'];
	 //echo $id;
	$query = "SELECT * FROM internships WHERE lastdate >= '".$id."'";
	$row = $db->select($query);
?>
<?php  while($jobs = $row->fetch_assoc()): ?>
		<div class="post-job" style="margin-top: 20px;">
			<div class="col-xs-9" style="margin-top: 20px;">
				...
				<table class="table">
					...
					<tr><th><?php echo $jobs["place"]; ?></th><th><?php echo $jobs["salary"]; ?></th><th><?php echo $jobs["lastdate"]; ?></th><th><?php echo $jobs["lastdate"]; ?></th></tr>
				</table>
			</div>
			<div class="col-xs-3">
