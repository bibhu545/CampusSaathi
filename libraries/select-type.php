...
<?php
	...
	$db = new Database();
	$id = $_REQUEST['did'];
	$query = "SELECT * FROM jobs as t1 left join location as t2 on t1.place=t2.place_id WHERE type ='".$id."'";
	$row = $db->select($query);
?>
		...
		<div class="post-job">
			<div class="col-xs-9">
				...
				<table class="table">
					...
					<tr><th><?php echo $jobs["location"]; ?></th><th><?php echo $jobs["salary"]."/mo"; ?></th><th><?php echo $jobs["lastdate"]; ?></th><th><?php echo $jobs["lastdate"]; ?></th></tr>
				</table>
			</div>
			<div class="col-xs-3">
